...
import { Target, Layers, MessageSquare, PenTool, Edit3, Loader2, Search, X, List, Volume2, BookOpen, Eye, EyeOff } from 'lucide-react';
...
// ══════════════════════════════════════════
// FIX 1: КНОПКА СКРЫТИЯ ПИНЬИНЬ
// showPinyin пробрасывается во все карточки
// ══════════════════════════════════════════

// ══════════════════════════════════════════
// РЕЖИМ: СПИСОК СЛОВ
// ══════════════════════════════════════════
function WordListMode({ vocab, onGoToDraw, showPinyin }) {
  ...
  const playAudio = (w) => { setPlaying(w.id); speak(w.h); setTimeout(() => setPlaying(null), 1200); };
  const toggleFlip = (id) => setFlipped(prev => ({ ...prev, [id]: !prev[id] }));

  return (
    <div className={styles.animFadeUp}>
      <style>{`
        .wl-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-radius: 18px; padding: 16px 20px; display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 16px; transition: all 0.18s; cursor: default; }
        .wl-card:hover { background: rgba(255,255,255,0.055); border-color: rgba(255,255,255,0.12); transform: translateY(-1px); }
        .wl-hz { font-family: 'Noto Serif SC', serif; font-size: 36px; line-height: 1; color: white; white-space: nowrap; letter-spacing: 4px; text-shadow: 0 0 20px rgba(192,57,43,0.3); }
        .wl-btn { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; color: rgba(255,255,255,0.5); width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.15s; flex-shrink: 0; }
        .wl-btn:hover { background: rgba(255,255,255,0.1); color: white; }
        .wl-draw-btn { background: rgba(192,57,43,0.1); border: 1px solid rgba(192,57,43,0.25); border-radius: 10px; color: #E87060; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.15s; flex-shrink: 0; }
        .wl-draw-btn:hover { background: rgba(192,57,43,0.2); border-color: rgba(192,57,43,0.5); color: #FF8070; box-shadow: 0 0 12px rgba(192,57,43,0.3); }
        .wl-meaning { overflow: hidden; max-height: 0; opacity: 0; transition: max-height 0.25s ease, opacity 0.2s; }
        .wl-meaning.open { max-height: 80px; opacity: 1; }
        @media (max-width: 600px) { .wl-card { gap: 10px; padding: 12px 14px; } .wl-hz { font-size: 26px; letter-spacing: 2px; } }
      `}</style>

      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, gap: 12 }}>
        <div>
          <div style={{ fontSize: 18, fontWeight: 700, color: "white" }}>Список слов</div>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", marginTop: 2 }}>{vocab.length} слов · нажми чтобы раскрыть</div>
        </div>
        <div style={{ position: "relative", flexShrink: 0 }}>
          <Search size={14} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "rgba(255,255,255,0.3)" }} />
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Фильтр..."
          ...
          {search && <button onClick={() => setSearch("")} style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "rgba(255,255,255,0.4)", cursor: "pointer", padding: 0 }}><X size={13} /></button>}
        </div>
      </div>
          ...
          return (
            <div key={w.id} style={{ animationDelay: `${i * 0.02}s` }}>
              <div className="wl-card" onClick={() => toggleFlip(w.id)}>
                ...
                <div>
                  {/* FIX 1: пиньинь скрывается если showPinyin=false */}
                  {showPinyin && <div style={{ fontSize: 15, color: "#E74C3C", fontWeight: 600, letterSpacing: "0.5px" }}>{w.p}</div>}
                  <div style={{ fontSize: 14, color: "white", marginTop: showPinyin ? 2 : 0 }}>{w.ru}</div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }} onClick={e => e.stopPropagation()}>
                  <button className="wl-btn" onClick={() => playAudio(w)} title="Прослушать" style={playing === w.id ? { background: "rgba(231,76,60,0.15)", borderColor: "rgba(231,76,60,0.4)", color: "#E74C3C" } : {}}>
                    <Volume2 size={15} />
                  </button>
                  <button className="wl-draw-btn" onClick={() => onGoToDraw(w)} title="Рисование"><PenTool size={15} /></button>
                </div>
              </div>
            </div>
          );
        })}
        {filtered.length === 0 && <div style={{ textAlign: "center", padding: "50px 0", color: "rgba(255,255,255,0.3)" }}><div style={{ fontSize: 36, marginBottom: 10 }}>🔍</div><div>Ничего не найдено</div></div>}
      </div>
    </div>
  );
}
...
// ══════════════════════════════════════════
// QUIZ — добавлен showPinyin
// ══════════════════════════════════════════
function QuizMode({ vocab, onScore, onFinish, showPinyin }) {
  ...
  if (!item) return null;
  const check = (opt) => { if (answered) return; setAnswered(true); setChosen(opt); onScore(opt === correctAns); };
  const next = () => { if (idx + 1 >= items.current.length) return onFinish(); setIdx(i => i + 1); setAnswered(false); setChosen(null); };
    ...
    <div className={styles.animFadeUp}>
      ...
      <div className={styles.animCardIn} style={{ background: '#FAF7F2', color: '#0D0D0D', borderRadius: 28, padding: "36px 32px", textAlign: "center", marginBottom: 20, position: "relative", boxShadow: "0 20px 60px rgba(0,0,0,.4)" }}>
        {dir === "p2r" ? (
          <>
            ...
            {/* FIX 1 */}
            {showPinyin && <div style={{ fontSize: 20, color: '#C0392B', fontWeight: 600, letterSpacing: 2, marginBottom: 10 }}>{item.p}</div>}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
        {options.map(opt => {
          ...
          return <button key={opt} disabled={answered} onClick={() => check(opt)} className={`${styles.optBtn} ${anim}`} style={{ background: bg, border: `2px solid ${border}`, color, borderRadius: 16, padding: "15px 12px", fontSize: 14, fontWeight: 500, cursor: answered ? "default" : "pointer" }}>{opt}</button>;
        })}
      </div>
...
// ══════════════════════════════════════════
// FLASHCARD — добавлен showPinyin
// ══════════════════════════════════════════
function FlashcardMode({ vocab, onScore, onFinish, showPinyin }) {
  const items = useRef(shuffle(vocab).slice(0, 10));
  const [idx, setIdx] = useState(0);
  const [flipped, setFlipped] = useState(false);
  const item = items.current[idx];

  const answer = (knew) => { onScore(knew); if (idx + 1 >= items.current.length) return onFinish(); setFlipped(false); setTimeout(() => setIdx(i => i + 1), 150); };
  if (!item) return null;
      ...
      <div className={styles.fcWrap} style={{ marginBottom: 20 }}>
        <div className={`${styles.fcInner} ${flipped ? styles.flipped : ""}`} onClick={() => setFlipped(true)}>
          <div className={styles.fcFace} style={{ background: '#FAF7F2', color: '#0D0D0D' }}>
            ...
            {/* FIX 1 */}
            {showPinyin && <div style={{ fontSize: 20, color: '#C0392B', fontWeight: 600, letterSpacing: 2, marginTop: 8 }}>{item.p}</div>}
...
// ══════════════════════════════════════════
// TYPE — добавлен showPinyin
// ══════════════════════════════════════════
function TypeMode({ vocab, onScore, onFinish, showPinyin }) {
  const items = useRef(shuffle(vocab).slice(0, 10));
  const [idx, setIdx] = useState(0);
  ...
  const next = () => { setIdx(i => i + 1); setInput(""); setAnswered(false); if (idx + 1 >= items.current.length) onFinish(); };
  return (
    <div className={styles.animFadeUp}>
      <ProgressBar value={(idx / items.current.length) * 100} />
      <div className={styles.animCardIn} style={{ background: '#FAF7F2', color: '#0D0D0D', borderRadius: 24, padding: "32px 28px", textAlign: "center", marginBottom: 18, boxShadow: "0 20px 60px rgba(0,0,0,.4)" }}>
        ...
        {/* FIX 1 */}
        {showPinyin && <div style={{ fontSize: 20, color: '#C0392B', fontWeight: 600, letterSpacing: 2, marginBottom: 8 }}>{item.p}</div>}
...
function DrawMode({ vocab, onScore, onFinish, focusWord }) {
  const ordered = focusWord ? [focusWord, ...vocab.filter(v => v.id !== focusWord.id)] : vocab;
  ...
  if (!word) return <div style={{ textAlign: 'center', padding: 40, color: "rgba(255,255,255,0.4)" }}>Нет слов для практики.</div>;

  const handleCharComplete = (isCorrect) => {
    onScore(isCorrect);
    const isLastChar = charIdx + 1 >= chars.length;
    if (isLastChar) {
      if (wordIdx + 1 >= items.current.length) onFinish();
      else { setWordIdx(i => i + 1); setCharIdx(0); }
  ...
  return (
    <div className={styles.animFadeUp}>
      ...
      <div style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 18, padding: "16px 20px", marginBottom: 16 }}>
        <div style={{ fontSize: 11, letterSpacing: 2, color: "rgba(255,255,255,.3)", textTransform: "uppercase", marginBottom: 12 }}>Слово {wordIdx + 1} из {totalWords}</div>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 10, flexWrap: "wrap" }}>
          {chars.map((ch, i) => {
            const isDone = i < charIdx; const isCurrent = i === charIdx;
            return (
              <div key={i} style={{ textAlign: "center", transition: "all .25s" }}>
                <div style={{ fontFamily: "'Noto Serif SC', serif", fontSize: 44, lineHeight: 1, padding: "8px 14px", borderRadius: 14, border: `2px solid ${isCurrent ? "#C0392B" : isDone ? "rgba(93,138,110,.4)" : "rgba(255,255,255,.08)"}`, background: isCurrent ? "rgba(192,57,43,.12)" : isDone ? "rgba(93,138,110,.1)" : "rgba(255,255,255,.03)", color: isDone ? "#7EC89A" : isCurrent ? "white" : "rgba(255,255,255,.3)", boxShadow: isCurrent ? "0 0 18px rgba(192,57,43,.25)" : "none", transition: "all .25s" }}>
                  {isDone ? "✓" : ch}
                </div>
                <div style={{ fontSize: 11, marginTop: 5, letterSpacing: "0.5px", color: isCurrent ? "#C0392B" : isDone ? "rgba(93,138,110,.6)" : "rgba(255,255,255,.2)" }}>
                  {word.p.split(" ")[i] || ""}
                </div>
              </div>
            );
      ...
      <div style={{ textAlign: "center", marginBottom: 10 }}>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,.4)" }}>
          ...
          {chars.length > 1 && <span style={{ marginLeft: 8, color: "#C0392B", fontWeight: 600 }}>{charIdx + 1} / {chars.length}</span>}
        </div>
      </div>
      <HanziDraw key={`${word.id}-${charIdx}`} character={currentChar} onComplete={handleCharComplete} />
    </div>
  );
}
...
const HSK_COLORS = { 1:"#7EC89A", 2:"#60B4D0", 3:"#D4A017", 4:"#E8A060", 5:"#E74C3C", 6:"#C0392B" };
const HSK_RGB    = { 1:"93,138,110", 2:"96,180,208", 3:"212,160,23", 4:"232,160,96", 5:"231,76,60", 6:"192,57,43" };

// ══════════════════════════════════════════
// FILTER BAR — все 5 фиксов здесь
// ══════════════════════════════════════════
function FilterBar({ allWords, selectedHsk, setSelectedHsk, selectedCats, setSelectedCats, selectedWords, setSelectedWords }) {
  ...
  // FIX 6: режим выбора категорий
  const [selectMode, setSelectMode] = useState(false);
  // FIX 5: открытый HSK в режиме выбора
  const [openHsk, setOpenHsk] = useState(null);
  ...
  // FIX 3+4: addWord добавляет в selectedWords, не сбрасывает категории
  const addWord = (word) => {
  ...
  // FIX 5: toggleCat НЕ сбрасывает при смене HSK
  const toggleCat = (cat) => {
  ...
  // Текущие категории для показа (обычный режим — только выбранного HSK)
  ...
  return (
    <div style={{ borderBottom: "1px solid rgba(255,255,255,.07)", background: "#111" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 16px", overflowX: "auto", scrollbarWidth: "none" }}>
        ...
        {[1,2,3,4,5,6].map(n => (
          <button key={n}
            onClick={() => {
              if (selectMode) {
                // FIX 6: в режиме выбора — нажатие на HSK открывает/закрывает его категории
                setOpenHsk(prev => prev === n ? null : n);
              } else {
                // Обычный режим — переключает фильтр, НЕ сбрасывает selectedCats (FIX 5)
                setSelectedHsk(n);
              }
            }}
            style={{
              ...
              background: selectedHsk === n ? `rgba(${HSK_RGB[n]},.15)` : "transparent",
            ...
            HSK {n}
            {/* Показываем сколько категорий выбрано из этого HSK */}
            {selectMode && selectedCats.filter(c => Number(c.hsk) === n).length > 0 && (
              <span style={{ marginLeft: 5, background: HSK_COLORS[n], color: "#000", fontSize: 10, fontWeight: 900, padding: "1px 5px", borderRadius: 8 }}>
                {selectedCats.filter(c => Number(c.hsk) === n).length}
              </span>
            )}
          </button>
        ))}

        {/* FIX 6: Кнопка "Выбрать" */}
        <button
          onClick={() => { setSelectMode(s => !s); setOpenHsk(null); }}
          style={{
            marginLeft: 6, padding: "6px 14px", borderRadius: 20, whiteSpace: "nowrap",
            border: `1.5px solid ${selectMode ? "#D4A017" : "rgba(255,255,255,.1)"}`,
            background: selectMode ? "rgba(212,160,23,.15)" : "transparent",
            color: selectMode ? "#D4A017" : "rgba(255,255,255,.4)",
            fontSize: 12, fontWeight: 700, cursor: "pointer", transition: "all .18s"
          }}>
          {selectMode ? "✓ Выбор вкл" : "Выбрать"}
        </button>
        ...
        <div ref={dropRef} style={{ marginLeft: "auto", position: "relative", flexShrink: 0 }}>
          ...
          <input ref={searchRef} value={searchQ} onChange={e => setSearchQ(e.target.value)} onFocus={() => searchResults.length > 0 && setDropOpen(true)} placeholder="Поиск по пиньинь..."
            style={{ background: "rgba(255,255,255,.06)", border: `1.5px solid ${dropOpen ? "rgba(192,57,43,.5)" : "rgba(255,255,255,.1)"}`, borderRadius: 20, padding: "7px 14px 7px 30px", color: "white", fontSize: 13, outline: "none", width: 180, fontFamily: "'DM Sans',sans-serif", transition: "border-color .2s" }} />
          {searchQ && <button onClick={() => { setSearchQ(""); setSearchResults([]); setDropOpen(false); }} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "rgba(255,255,255,.4)", cursor: "pointer", padding: 0, display: "flex" }}><X size={13} /></button>}

          {dropOpen && searchResults.length > 0 && (
            <div style={{ position: "fixed", top: (() => { if (searchRef.current) { const r = searchRef.current.getBoundingClientRect(); return r.bottom + 6; } return 100; })(), right: 16, width: 260, background: "#1A1A1A", border: "1px solid rgba(255,255,255,.12)", borderRadius: 16, zIndex: 9999, overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,.8)" }}>
              <div style={{ padding: "8px 12px 6px", fontSize: 10, letterSpacing: 1.5, color: "rgba(255,255,255,.25)", textTransform: "uppercase", borderBottom: "1px solid rgba(255,255,255,.06)" }}>Найдено {searchResults.length} слов</div>
              {searchResults.map(w => (
      ...
      {/* FIX 6: В режиме выбора показываем категории открытого HSK */}
      {selectMode && openHsk && (
        <div style={{ padding: "6px 16px 12px", background: "rgba(255,255,255,.02)", borderTop: "1px solid rgba(255,255,255,.05)" }}>
          <div style={{ fontSize: 10, letterSpacing: 2, color: HSK_COLORS[openHsk], textTransform: "uppercase", marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
            <span>HSK {openHsk} — выбери категории</span>
            {/* FIX 6: кнопка "выбрать весь HSK" */}
            <button
              onClick={() => {
                const hskCatsAll = categories.filter(c => Number(c.hsk) === openHsk);
                const allSelected = hskCatsAll.every(c => selectedCats.find(sc => sc.id === c.id));
                if (allSelected) {
                  setSelectedCats(prev => prev.filter(c => Number(c.hsk) !== openHsk));
                } else {
                  const toAdd = hskCatsAll.filter(c => !selectedCats.find(sc => sc.id === c.id));
                  setSelectedCats(prev => [...prev, ...toAdd]);
                }
              }}
              style={{ fontSize: 10, padding: "2px 8px", borderRadius: 10, border: "1px solid rgba(255,255,255,.15)", background: "rgba(255,255,255,.06)", color: "rgba(255,255,255,.5)", cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>
              {categories.filter(c => Number(c.hsk) === openHsk).every(c => selectedCats.find(sc => sc.id === c.id)) ? "Снять все" : "Все HSK " + openHsk}
            </button>
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {categories.filter(c => Number(c.hsk) === openHsk).map(cat => {
              const isOn = !!selectedCats.find(c => c.id === cat.id);
              return (
                <button key={cat.id}
                  onClick={() => toggleCat(cat)}
                  style={{ padding: "6px 13px", borderRadius: 20, border: `1.5px solid ${isOn ? HSK_COLORS[openHsk] : "rgba(255,255,255,.1)"}`, background: isOn ? `rgba(${HSK_RGB[openHsk]},.18)` : "rgba(255,255,255,.04)", color: isOn ? "white" : "rgba(255,255,255,.5)", fontSize: 12, fontWeight: isOn ? 600 : 400, cursor: "pointer", transition: "all .15s", display: "flex", alignItems: "center", gap: 5 }}>
                  {cat.icon && <span>{cat.icon}</span>}
                  {cat.nameRu}
                  {isOn && <span style={{ marginLeft: 3, opacity: .7 }}>✓</span>}
                </button>
              );
            })}
            {categories.filter(c => Number(c.hsk) === openHsk).length === 0 && (
              <span style={{ fontSize: 12, color: "rgba(255,255,255,.25)" }}>Нет категорий для HSK {openHsk}</span>
            )}
          </div>
        </div>
      )}

      {/* Обычный режим — категории текущего HSK */}
      {!selectMode && hskCats.length > 0 && (
          ...
          {hskCats.map(cat => {
            ...
            return (
              <button key={cat.id} onClick={() => toggleCat(cat)}
                style={{ padding: "5px 12px", borderRadius: 20, whiteSpace: "nowrap", border: `1.5px solid ${isOn ? HSK_COLORS[selectedHsk] : "rgba(255,255,255,.08)"}`, background: isOn ? `rgba(${HSK_RGB[selectedHsk]},.12)` : "rgba(255,255,255,.03)", color: isOn ? "white" : "rgba(255,255,255,.4)", fontSize: 12, fontWeight: 500, cursor: "pointer", transition: "all .15s", display: "flex", alignItems: "center", gap: 5 }}>
                {cat.icon && <span>{cat.icon}</span>}
                {cat.nameRu}
      ...
      {/* Активные теги — FIX 3+4+5: показываем и категории и слова вместе */}
...
export default function TrainerPage() {
  ...
  // FIX 5: selectedCats НЕ сбрасывается при смене HSK
  const [selectedHsk, setSelectedHsk] = useState(1);
  const [selectedCats, setSelectedCats] = useState([]);
  const [selectedWords, setSelectedWords] = useState([]);
  const [drawFocusWord, setDrawFocusWord] = useState(null);

  // FIX 1: глобальный стейт пиньинь
  const [showPinyin, setShowPinyin] = useState(true);

  const isLocked = !user && mode !== 'quiz' && mode !== 'wordlist';

  // FIX 3+4: vocab = категории + слова из поиска объединяются
  const activeVocab = (() => {
    const catIds = selectedCats.map(c => c.id);
    const catWords = catIds.length > 0 ? words.filter(w => catIds.includes(w.categoryId)) : [];
    // FIX 3: минимум 1 слово из поиска уже работает
    const searchWords = selectedWords.filter(w => !catWords.find(cw => cw.id === w.id));
    // FIX 4: объединяем категории + поиск
    if (catWords.length > 0 || searchWords.length > 0) {
      return [...catWords, ...searchWords];
    }
    // Иначе — все слова выбранного HSK
    return words.filter(w => Number(w.hsk) === selectedHsk);
  })();

  const vocab = activeVocab.length >= 1 ? activeVocab : words.filter(w => Number(w.hsk) === selectedHsk);

  const handleGoToDraw = (word) => { setDrawFocusWord(word); setMode("draw"); setCorrect(0); setWrong(0); setStreak(0); setFinished(false); setSessionKey(p => p + 1); };
  ...
  function restart() { setCorrect(0); setWrong(0); setStreak(0); setFinished(false); setSessionKey(p => p + 1); setDrawFocusWord(null); }
  function finishSession() { setFinished(true); }

  // FIX 5: НЕ сбрасываем selectedCats при смене HSK
    ...
    return (
      <div className={styles.root}>
        ...
        <div style={{ textAlign: 'center', padding: 100 }}>
          ...
          <Link href="/admin-panel" style={{ background: '#C0392B', color: "white", border: "none", borderRadius: 20, padding: "15px 40px", fontSize: 18, fontWeight: 700, textDecoration: "none", display: "inline-block" }}>Добавить слова в Админке</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={styles.root}>
      ...
      <FilterBar allWords={words} selectedHsk={selectedHsk} setSelectedHsk={setSelectedHsk} selectedCats={selectedCats} setSelectedCats={setSelectedCats} selectedWords={selectedWords} setSelectedWords={setSelectedWords} />

      {/* Мобайл: нижняя панель */}
      ...
      <div className={styles.trainerGrid}>
        ...
        <aside className={styles.sidebar}>
          ...
          {/* FIX 1: кнопка скрытия пиньинь в сайдбаре */}
          <div style={{ margin: "12px 16px 0" }}>
            <button
              onClick={() => setShowPinyin(s => !s)}
              style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderRadius: 12, border: `1.5px solid ${showPinyin ? "rgba(255,255,255,.1)" : "rgba(192,57,43,.4)"}`, background: showPinyin ? "rgba(255,255,255,.03)" : "rgba(192,57,43,.1)", color: showPinyin ? "rgba(255,255,255,.5)" : "#E87060", cursor: "pointer", fontSize: 12, fontWeight: 600, transition: "all .2s", fontFamily: "'DM Sans',sans-serif" }}>
              {showPinyin ? <Eye size={14} /> : <EyeOff size={14} />}
              {showPinyin ? "Пиньинь виден" : "Пиньинь скрыт"}
            </button>
          </div>

          <div style={{ margin: "12px 16px 0", padding: "12px", background: "rgba(255,255,255,.03)", borderRadius: 12, border: "1px solid rgba(255,255,255,.06)" }}>
            ...
            <div style={{ fontSize: 11, color: "rgba(255,255,255,.35)", marginTop: 2 }}>
              {selectedWords.length > 0 && selectedCats.length > 0 ? "Категории + поиск" : selectedWords.length > 0 ? "Выбранные слова" : selectedCats.length > 0 ? "По категориям" : `HSK ${selectedHsk} — все`}
            </div>
          </div>
        </aside>
          ...
          <div style={{ filter: isLocked ? 'blur(6px)' : 'none', pointerEvents: isLocked ? 'none' : 'auto', opacity: isLocked ? 0.4 : 1, transition: 'all 0.3s' }}>
            {finished && !isLocked ? (
              ...
              <div key={`${mode}-${sessionKey}`}>
                {mode === "wordlist"  && <WordListMode  vocab={vocab} onGoToDraw={handleGoToDraw} showPinyin={showPinyin} />}
                {mode === "quiz"      && <QuizMode      vocab={vocab} onScore={handleScore} onFinish={finishSession} showPinyin={showPinyin} />}
                {mode === "flashcard" && <FlashcardMode vocab={vocab} onScore={handleScore} onFinish={finishSession} showPinyin={showPinyin} />}
                {mode === "dialogue"  && <DialogueMode               onScore={handleScore} onFinish={finishSession} />}
                {mode === "type"      && <TypeMode      vocab={vocab} onScore={handleScore} onFinish={finishSession} showPinyin={showPinyin} />}
        ...
        <aside className={styles.rightSidebar}>
          ...
          <div style={{ marginTop: 16, padding: "12px", background: "rgba(255,255,255,.03)", borderRadius: 12, border: "1px solid rgba(255,255,255,.06)" }}>
            ...
            <div style={{ fontSize: 12, color: "rgba(255,255,255,.5)" }}>
              ...
              {selectedWords.length > 0 && <span style={{ color: "#E87060" }}> · {selectedWords.length} сл.</span>}
            </div>
          </div>

          {mode !== "wordlist" && (
            <button onClick={() => { setMode("wordlist"); restart(); }}
